...
//https://stackoverflow.com/questions/44576090/erlang-c-node-erl-errno-symbol-not-found-error
//that is why we need next strange define
#ifndef _REENTRANT
 ...
 * map_user.c
 *
 * this application is like a gateway between gctload and erlang:
 * receive messages from gctload, transfer it to erlang
 * and send replies from erlang back to gctload.
 *
 * two-way gateway.
 *
 * designed to run in docker container.
 ...
 * Always returns TBD.
 */
char *timestamp(char *currenttime) {
...
* received messages transferred to main pthread
* endless loop
*
**********************************************************************/
 ...
 while(1)
   {
     ...
     switch (got)
       {
       case ERL_TICK:
         data = 7; /*ERL_TICK notificatin send to main */
	 ...
	 //}
         ...
         break;
       case ERL_ERROR:
         //         printf("ERL_ERROR: erl errno = %d\n", erl_errno);
         fprintf(stderr, "MAP_USER: Error %d returned by erl_receive_msg\n", erl_errno);  
         data = 8; /* ERL_ERROR send to main pthread */
         result = write (fd1[1], &data, 1);
         sleep(5);        
         //exit(2);
         break;
...
/*
  ...
  if ((m = getm((u16)ptr->msg_type, ptr->dlg_id, NO_RESPONSE, (u16)(ptr->payload_length))) != 0)
  {
     ...
     */
    //uncomment this if need print msg
...
int mapu_erl_send(MSG *m)
{
  ...
  erl_tuple_obj[2] = erl_mk_binary( (const char *)pptr, m->len);
  ...
  if  (m->hdr.type == MAP_MSG_DLG_IND)
	...
	case MAPDT_OPEN_CNF:
	  ...
	  break;
        ...
        case MAPDT_P_ABORT_IND:
          erl_tuple_obj[0] = erl_mk_atom("mapdt_pabort_ind");
	  break;

        case MAPDT_NOTICE_IND:
          erl_tuple_obj[0] = erl_mk_atom("mapdt_notice_ind");
	  break;
   ...
   //TODO
   //suppose that we should send here TC_U_ABORT to map - to release all resources and to abort dialogue
  if (!erl_reg_send(fd, "enode_broker", emsg))
    {
    fprintf( stderr, "MAP_USER: %s Error while sending msg to enode\n", timestamp(&currentTime[0]) );
    MAPU_disp_msg("MAP_USER: not_send_to_enode:", m);
    }
//Q1
...
char enode_name[32] = "enode@";
//  char str2[] = "World";

  strcat(enode_name, hostname);

  //  printf("enode name  = %s\n", &enode_name);
    //#define ERL_NODE "enode@localhost"
  
  erl_init(NULL, 0);


  ret = erl_connect_init(1,ERL_COOKIE,0);


  if (( fd = erl_connect(enode_name) ) < 0)
  //erl_err_quit("erl_connect") --- the following is the same, accordign to erl_err_quit description
    {
      fprintf(stderr, "MAP_USER: %s Error while connecting to %s\n", timestamp(&currentTime[0]), enode_name);  
      exit(1);
    }
  else
    fprintf(stderr, "MAP_USER: %s Connected to %s\n", timestamp(&currentTime[0]), enode_name);  
 ...
 tv.tv_sec = 20;
	...
	switch(p_order->msg_type)
	  ...
	  case MAP_MSG_DLG_IND:
	    ...
	    break;
          default:
            printf("received from erl receiver in main pthread = %d\n", buffer[0]);

            if (buffer[0] == 8)
                       {
                         ret =  erl_close_connection(fd);
                         //printf("ret close connection = %d\n", ret);

                         if (( fd = erl_connect(enode_name) ) < 0)
                           //erl_err_quit("erl_connect") --- the following is the same, accordign to erl_err_quit description
                           {
                             fprintf(stderr, "MAP_USER: %s Error while retry connect to %s\n", timestamp(&currentTime[0]), enode_name);  
                             //exit(1);
                           }
                         else
                           fprintf(stderr, "MAP_USER: %s Connected to %s\n", timestamp(&currentTime[0]), enode_name);  
